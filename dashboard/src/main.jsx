...
function App() {
  ...
  async function performAction(event) {
    ...
    try {
      ...
      if (currentAction.reveal) {
        ...
        setNotice('Captured prompts are visible for 60 seconds. No credential has been saved.');
      } else {
  ...
  const events = data?.central_events || [];
  ...
  function promptFor(event) {
    ...
    const captured = sensitive.central_events?.find(item => item.event_id && item.event_id === event.event_id)?.user_prompt;
  ...
  return <>
    <aside className="rail"><a className="brand" href="/dashboard/"><span className="brand-mark">B</span> BAP<span className="brand-caption">CONTROL PLANE</span></a>
      ...
      <div className="rail-foot"><span className="secure-dot"/> Bounded authority<br/><a href="/inspector?mode=live">Diagnostic inspector ↗</a></div>
    </aside>
    <main>
      ...
      <div className="endpoint-bar"><span>CONTROL PLANE <strong>{location.origin}</strong></span><span>{sensitive ? `Admin prompt reveal · ${Math.max(0, Math.ceil((revealUntil - now) / 1000))}s remaining` : 'Standard view · prompts protected'} · {lastRefresh ? elapsed(now - lastRefresh) : 'Not yet refreshed'}</span></div>
      ...
      <section id="registry" className="panel">
        ...
        <div className="toolbar"><div className="tabs" role="group" aria-label="Agent view"><button className={view === 'live' ? 'selected' : ''} onClick={() => setView('live')}>Live view</button><button className={view === 'history' ? 'selected' : ''} onClick={() => setView('history')}>All agents & history</button></div><label className="search-label"><span className="sr-only">Search agents</span><input type="search" placeholder="Search agent, user or host…" value={search} onChange={e => setSearch(e.target.value)}/></label></div>
        <div className="table-wrap"><table><thead><tr><th>Agent / instance</th><th>User</th><th>Host</th><th>Status</th><th>Last seen</th><th>Admin action</th></tr></thead><tbody>{filtered.map(agent => <tr key={agent.agent_id}>
          ...
          <td><Badge status={agent.presence.status}/>{agent.presence.status === 'deregistered' && view === 'live' && <small>Leaves view in {agent.presence.remaining}s</small>}</td>
          <td>{elapsed(agent.presence.age)}</td><td>{['active', 'revoked', 'stale'].includes(agent.presence.status) ? <button disabled={!connected || pending} className={agent.status !== 'revoked' ? 'danger-outline' : ''} onClick={() => openAction({ title: agent.status === 'revoked' ? 'Restore agent' : 'Revoke agent', path: '/control/agent/kill', body: { target: targetFor(agent), action: agent.status === 'revoked' ? 'restore' : 'revoke' }, impact: `${agent.agent_name || agent.app_id} · ${targetFor(agent)}` })}>{agent.status === 'revoked' ? 'Restore' : 'Revoke'}</button> : '—'}</td>
        </tr>)}</tbody></table></div>
        ...
        <footer className="panel-foot">Deregistered entries leave Live view after 30 seconds. History is retained for this server's registry lifetime. Stale means contact is overdue, not confirmed deregistration.</footer>
      </section>
      <section id="activity" className="panel"><div className="panel-title"><div><h2>Prompts & tool activity</h2><p>Captured intent alongside actual tool actions. No sample events.</p></div><div>{sensitive ? <button onClick={() => { setSensitive(null); setRevealUntil(0); }}>Hide prompts</button> : <button disabled={!connected || pending} onClick={() => openAction({ title: 'Reveal captured prompts', path: '/admin/inspector/data', reveal: true, impact: 'Show sensitive user prompts in this browser for 60 seconds.' })}>Reveal prompts · Admin</button>}</div></div>
        ...
        <footer className="panel-foot">Prompt text is shown only when the client supplies it. Admin reveal expires after 60 seconds; credentials are never saved in browser storage.</footer>
