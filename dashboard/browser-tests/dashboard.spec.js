...
test('real session: prompt protection, admin action, failed credential and restore', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Reveal prompts' }).click();
  ...
  await page.getByRole('button', { name: 'Hide prompts' }).click();
  await expect(page.getByText('Review the payment changes for mistakes.', { exact: true })).toHaveCount(0);
});

test('30 second departure retains history and renders untrusted names as text', async ({ page }) => {
  ...
  await page.goto('/dashboard/');
  await expect(page.getByText('No agents in this view.', { exact: false })).toBeVisible();
  await page.getByRole('button', { name: 'All agents & history' }).click();
